...
const esVendedorOAdmin = computed(() => sesion.value?.rol === "vendedor" || sesion.value?.rol === "administrador");
...
function asegurarPolling(id) {
  if (intervalo) return;
  intervalo = setInterval(() => cargarOferta(id), 3000);
}
...
async function crearOferta() {
  ...
  if (ok) {
    toast("Oferta relámpago iniciada", "success");
    await cargarOferta(props.productoId);
  } else {
...
async function finalizarOferta() {
  ...
  if (ok) {
    toast("Oferta finalizada", "success");
    await cargarOferta(props.productoId);
  } else {
...
<template>
  <div v-if="!cargando && (oferta || esVendedorOAdmin)" class="border-t border-neutral-200 pt-6 mt-2">
